---
title: 2026 Fastpitch Softball at SMLL
permalink: /softball/
tag: softball, 2026-season
toc: true
toc_icon: softball
toc_h_max: 3
#header:
#  overlay_image: /assets/images/smll-wildflowers-20260219-round.png
---

This page describes the 2026[^past] spring softball season at SMLL.

![Sierra Wildflowers Logo]({% link /assets/images/smll-wildflowers-20260219-round-trans.png %}){: .align-center .half-width }


## Divisions

### Common Rules

The following rules apply to all divisions:

- 5-run limit per team per inning[^runs]
- Pitchers may wear a face mask while in the pitching position.
- Pitchers are only allowed to pitch 2 consecutive innings, after which another
  pitcher must be substituted in for at least 1 inning. After this, the original
  pitcher may return.
- A team will have a minimum of 8 players at game time. A maximum of 2 players
  may be borrowed from the opponent to reach 8. A player may only be picked up
  3 times per season.
- Continuous batting order is used: all rostered players will bat, in roster order,
  whether or not they are playing a defensive position in the current inning.
- Parent or coach may serve as umpire if no official is available
- Runners are required to slide or attempt to avoid a defensive player on base if the play
  on the runner is close.[^sliding]


### 8U
{: .softball}

No 8U at SMLL this season! We didn't have enough players for more than 1 team.
{: .notice--info }


### 10U
{: .softball}

| **Ages**                  | 7-10<sup>*</sup> |
| **Ball**                  | 11" standard softball<sup>*</sup> |
| **Pitch Distance**        | 35'
| **Base Distance**         | 60'
| **Game Times**            | 75 minutes or 7 innings<sup>*</sup> |
| **Run Rules**             | 5 runs per team per half-inning<sup>*</sup> |

[:page_facing_up:]({% link /assets/files/2025-interleague-fastpitch-rules-10u.pdf %})
&nbsp;&nbsp;&nbsp;&nbsp;_*: ages[^ages], balls[^softballs], limits[^limit], runs[^runs]_
{: .small }

10U is our transition division, where girls begin to play with nearly all the
regular rules of fastpitch softball.



#### 10U Rule Highlights

:heavy_check_mark:
 [Uncaught 3rd strike rule](https://www.littleleague.org/university/articles/answering-the-questions-parents-ask/) **IS** APPLIED\
:x:
 [Infield fly rule](https://www.littleleague.org/university/articles/explanation-infield-fly-rule/) IS **NOT** APPLIED
{: .notice--success }

#### 10U Lead-Offs

Lead-off rules follow those of regular NSA fastpitch softball:

**Rule 8.8(u):** _The baserunner is out_ \
"when the baserunner(s) fails to keep contact with their base until
a legally pitched ball leaves the pitcher's hand. The ball is dead,
'NO PITCH' is declared and the baserunner(s) is declared out."
{: .notice--info }


### 12U
{: .softball}

| **Ages**                  | 11-12<sup>*</sup> |
| **Ball**                  | 12" standard softball<sup>*</sup> |
| **Pitch Distance**        | 40'
| **Base Distance**         | 60'
| **Game Times**            | 90 minutes or 7 innings<sup>*</sup> |
| **Run Rules**             | 5 runs per team per half-inning<sup>*</sup> |

[:page_facing_up:]({% link /assets/files/2025-interleague-fastpitch-rules-12u.pdf %})
&nbsp;&nbsp;&nbsp;&nbsp;_*: ages[^ages], balls[^softballs], limits[^limit], runs[^runs]_
{: .small }

Our 12U division has full NSA fastpitch softball rules.

#### 12U Rule Highlights

:heavy_check_mark:
 [Uncaught 3rd strike rule](https://www.littleleague.org/university/articles/answering-the-questions-parents-ask/) **IS** APPLIED\
:x:
 [Infield fly rule](https://www.littleleague.org/university/articles/explanation-infield-fly-rule/) IS **NOT** APPLIED
{: .notice--success }

### 15U
{: .softball}

| **Ages**                  | 12-15 |
| **Ball**                  | 12" standard softball<sup>*</sup> |
| **Pitch Distance**        | 43'
| **Base Distance**         | 60'
| **Game Times**            | 90 minutes or 7 innings<sup>*</sup> |
| **Run Rules**             | 5 runs per team per half-inning<sup>*</sup> |

[:page_facing_up:]({% link /assets/files/2025-interleague-fastpitch-rules-14u.pdf %})
&nbsp;&nbsp;&nbsp;&nbsp;_*: ages[^ages], balls[^softballs], limits[^limit]_
{: .small }

#### 15U Ground Rules

- 40 foot pitch distance may be used if a pitching rubber is not available at 43 feet

#### 15U Rule Highlights

:heavy_check_mark:
 [Uncaught 3rd strike rule](https://www.littleleague.org/university/articles/answering-the-questions-parents-ask/) **IS** APPLIED\
:heavy_check_mark:
 [Infield fly rule](https://www.littleleague.org/university/articles/explanation-infield-fly-rule/) **IS** APPLIED\
:heavy_check_mark: Metal cleats are allowed. **Rule 3.4, Equipment/Shoes:**
"Metal cleats or any type of shoe that has exposed metal on the
 sole other than the cleat or rivet are not legal in youth play in
 the 12u division and younger. Metal cleats are legal in 14u, 16u,
 18u....
{: .notice--success }

## Nearby Leagues

SMLL schedules interleague games throughout the season with:

[Mariposa Girls Softball](https://mariposagirlssoftball.com/){:target="_blank"} (aka _MGS_)[^partner]
: Games are played at [Mariposa County High School](https://goo.gl/maps/vpK4Aya9wA1KXuud8){:target="_blank"}: 5074 Old Hwy N, Mariposa, CA 95338

Other nearby leagues include:

* [Yosemite Little League](https://www.yosemitelittleleague.org/){:target="_blank"} (aka _YLL_)
* [Madera National Little League](https://www.maderanational.com/){:target="_blank"} (aka _Madera_)
* [Sunnyside Little League](http://www.sunnysidell.org/){:target="_blank"} (aka _Sunnyside_)


## Schedules

Please see our [softball schedules page]({% link _pages/schedules/softball.md %}) for game times and live scores.




----
_Footnotes_

[^ages]: **[Ages]** New for the 2025-26 season, division ages listed here are based on Sep-Aug age groups.
    See the [USSSA Softball age calculator](https://www.usssa.com/fastpitch/ageCalculator),
    and the [NSA announcement](https://playnsa.com/2025/07/nsa-changes-age-eligibility-for-new-season/).
    In prior seasons, SMLL aligning with Little League in using birth year divisions.

[^limit]: **[Time Limits]** For game limits in marked divisions, no new inning is started after the time
    or inning limit is reached. Once an inning has started, it must be finished regardless
    of time unless the home team is ahead. 

[^past]: **[Past Years]** For previous years, see: [2023]({% link _pages/softball/2023.md %}),
    [2024]({% link _pages/softball/2024.md %}).
    [2025 Softball]({% link _pages/softball/2025.md %}).

[^partner]: **[Partners]** MGS and SMLL have worked in close
    partnership each year to create a strong interleague program.

[^runs]: **[Run Limits]** 5-run limit is found in [LL Rule 5.07](https://www.littleleague.org/app/Rulebook/2/159):
    "... (Minor League: The side is retired when three offensive
    players are legally put out, called out by an umpire, or when all
    players on the roster have batted one time in the half-inning or
    when the offensive team scores five (5) runs."

[^rulebook]: **[Rulebooks]** There's no great way to quote a fastpitch rulebook online. NSA shares a PDF,
    Little League shares an app. We use our own divisional rule definitions, in coordination with
    [Mariposa Girls Softball](https://mariposagirlssoftball.org), that are taken from the [NSA rulebook](
    https://playnsa.com/wp-content/uploads/2025/12/2026-NSA-Fastpitch-Rulebook-P.pdf), but also
    align with many of the rules in the [Little League Rulebook](https://www.littleleague.org/app/Rulebook).
    Our rules are adjusted to be developmentally appropriate for our player community this year.

[^softballs]: **[Softballs]** .47 COR, 375 lb compression, 11" or 12" circumference

[^sliding]: **[Sliding]** [There is no "must slide" rule](
    https://www.umpirebible.com/index.php/rules-base-running/force-play-slide-illegal-slide).
    Little League says runners must "slide or attempt to avoid". NSA is more colorful with
    **Rule 8.7:** "When a defensive player has the ball and is waiting for the runner and
    the runner remains on his/her feet and deliberately, with great force, crashes into the
    defensive player, the runner is declared out. The ball is dead, and all other runners must
    return to the last base touched at the time of the collision."
